    ...
    for fold, (train_loader, val_loader) in enumerate(data_loaders):
        ...
        if fold + 1 >= 1:
            ...
            for epoch in range(cfg.epochs):
                ...
                start = time.time()
                test0_acc, test0_loss, test0_sens, test0_spe, test0_ppv, test0_npv, test0_auc = \
                    train_val_epoch.val_cla_epoch(model, loss_fn, optimizer, inter_test_loader, device)
                end = time.time()
                print(
                    "内部测试集准确率为:test_acc:{:.5f}, test_loss:{:.5f}, test_sens:{:.5f}, test_spe:{:.5f}, test_ppv:{:.5f}, test_npv:{:.5f}, test_auc:{:.5f} "
                    "test_time:{:.0f}m, {:.0f}s".format(
                        test0_acc, test0_loss, test0_sens, test0_spe, test0_ppv, test0_npv, test0_auc,
                        (end - start) // 60, (end - start) % 60))

                start = time.time()
                test_acc, test_loss, test_sens, test_spe, test_ppv, test_npv, test_auc = \
                    train_val_epoch.val_cla_epoch(model, loss_fn, optimizer, exter_test_loader, device)
                end = time.time()
                print(
                    "外部测试集准确率为:test_acc:{:.5f}, test_loss:{:.5f}, test_sens:{:.5f}, test_spe:{:.5f}, test_ppv:{:.5f}, test_npv:{:.5f}, test_auc:{:.5f} "
                    "test_time:{:.0f}m, {:.0f}s".format(
                        test_acc, test_loss, test_sens, test_spe, test_ppv, test_npv, test_auc, (end - start) // 60,
                                                                                                (end - start) % 60))
